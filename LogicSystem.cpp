...
LogicSystem::LogicSystem() {
	...
	RegPost("/reset_pwd", [](std::shared_ptr<HttpConnection> connection) {
		...
		//更新密码为最新密码
		bool b_up = MysqlMgr::GetInstance()->UpdataPwd(name, pwd);
		if (!b_up) {
...
//	//用户登录逻辑
	RegPost("/user_login", [](std::shared_ptr<HttpConnection>connection) {
		auto str_body = boost::beast::buffers_to_string(connection->_request.body().data());
		std::cout << "receive the data is:" << str_body << std::endl;
		connection->_response.set(http::field::content_type, "text/json");
		Json::Value root;
		Json::Reader reader;
		Json::Value src_root;
		bool parse_success = reader.parse(str_body, src_root);
		if (!parse_success) {
			std::cout << "Failed to parse JSON data!" << std::endl;
			root["error"] = ErrorCodes::Error_Json;
			std::string jsonstr = root.toStyledString();
